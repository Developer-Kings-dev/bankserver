...
const accountSchema = new Schema({
  ...
  firstName: String,
  lastName: String,
  dailyWithdrawalLimit: { type: Number, default: 5000 },
...
app.post('/banks/', async (req, res) => {
  ...
  try {
    // Ensure unique account details
    const existingAccount = await Account.findOne({ firstName, lastName });
    if (existingAccount) {
      return res.status(400).json({ error: 'Account with these details already exists' });
    }

    // Generate unique account number
    const accountNumber = uuidv4().replace(/-/g, '').slice(0, 10); //10 unique account number
...
app.put('/banks/:id', async (req, res) => {
  ...
  const accountId = req.params.id;
  try {
    const account = await Account.findByIdAndUpdate(accountId, { firstName, lastName }, { new: true, runValidators: true });
    if (!account) {
      return res.status(404).json({ error: 'Account not found' });
    }
    res.status(200).json(account);
